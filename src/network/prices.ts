...
const apiUrl = { pyth: 'https://hermes.pyth.network', odin: 'https://odin-sport.quantena.org' }; // can add others here
...
export async function getPriceUpdates(traderAPI: TraderInterface, symbol: string): Promise<PriceUpdateResponseI[]> {
  ...
  const odinIds = priceIds.filter((_id, i) => !isPyth[i]);
  ...
  if (odinIds.length > 0) {
    const query =
      apiUrl.odin + '/v2/updates/price/latest?ids[]=' + priceIds.join('&ids[]=') + '&ignore_invalid_price_ids=true';
    const odinResponse = await fetch(query, getRequestOptions()).then((data) => {
      if (!data.ok) {
        console.error({ data });
        throw new Error(data.statusText);
      }
    ...
    console.log({ odinResponse });

    const updateData = odinResponse.binary.data.map<Hex>((data) => `0x${data}`);

    resp.push({
      address: contractAddress.pyth[Number(traderAPI.chainId)],
      ids: odinResponse.parsed.map((p) => (p.id.startsWith('0x') ? p.id : `0x${p.id}`) as `0x${string}`),
      updateData: updateData,
      publishTimes: odinResponse.parsed.map((p) => BigInt(p.price.publish_time)),
    });
  }
  // can add others here
